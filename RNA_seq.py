...
def cmdLine():
    ...
    ##creating genome index for alignemnt
        ...
        if args.genome_index == 'FALSE':
            index_cmd = ['STAR', '--runThreadN', args.thread_core
                , '--runMode genomeGenerate', '--genomeDir', args.genome_dir, '--genomeFastaFiles', args.fasta_dir,
                         '--sjdbGTFfile', args.gtf_dir]
        ...
        align_cmd = ['STAR', '--runThreadN', args.thread_core, '--genomeDir', args.genome_dir, '--runMode alignReads',
    ...
    ###generating the count matrix
        ...
        count_cmd = ['featureCounts', '-F GTF --primary -t exon -g gene_id --minOverlap 10', '-T', args.thread_core,
                     '-s', args.strandness, '-a', args.gtf_dir, '-o', count_output, bam]
